...
if (isset($_GET["load"])) {
    ...
    $sql = "SELECT a.*, b.nama as bahan, c.nama as supplier, d.harga, (a.jumlah_masuk-a.jumlah_retur) as bahan_masuk FROM `bahan_masuk` a ";
...
//    print_r($sql);die;
    ...
    if ($result = $mysql->query($sql)) {
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_array()) {
                $grandPrice = ((int) $row[9]) * ((int) $row[10]);
                $data[] = array(
                    "id" => $row[0],
                    ...
                    "jumlah_masuk" => $row[3],
                    "bahan_masuk" => $row[10],
                    "total" => number_format($grandPrice),
                    "tanggal_masuk" => $row[4],
                    "date_add" => $row[6],
                    "bahan" => $row[7],
                    "supplier" => $row[8],
    ...
    if ($id == "") {
        $sql = "INSERT INTO `bahan_masuk` (`id_supplier_bahan`, `jumlah_retur`, `jumlah_masuk`, `tanggal_masuk`) ";
        $sql .= "VALUES ('$supplier','$jumlah_retur','$jumlah_masuk','$tanggal_masuk')";
        $res["msg"] = "Bahan masuk berhasil ditambahkan.";

    } else {
        $sql = "UPDATE `bahan_masuk` SET `id_supplier_bahan`='$supplier', `jumlah_retur`='$jumlah_retur', `jumlah_masuk`='$jumlah_masuk', `tanggal_masuk`='$tanggal_masuk' WHERE `id` = '$id'";
